    ...
    return `
        <a-entity id="${id}">
            ...
            ${_.range(0,359.999,stepLongitude).map(v=>
                `<a-entity text-geometry="value: ${(v*maxLongitude/360)+longitudeUnit}; ${textSettings}" material="color: ${color}; opacity: ${opacity}; transparent: true;" mixin="FontGeom" spherical-coords="longitude: ${v}; latitude: 0; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}"></a-entity>`
            ).join('')}
            ...
            ${_.without(_.range(-80,80.001,10),0).map(v=>
                `<a-entity text-geometry="value: ${v}; ${textSettings}"  material="color: ${color}; opacity: ${opacity}; transparent: true;" mixin="FontGeom" spherical-coords="longitude: 0; latitude: ${v}; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}"></a-entity>`    
            ).join('')}
        </a-entity>
    `;
};
...
const html = `
    ...
    <a-scene 
        ...
        <a-assets>
            <a-asset-item id="Hyperspace_Font" src="server/font/hyperspace/Hyperspace_Regular.json"></a-asset-item>
            <a-mixin id="FontGeom" text-geometry="font: #Hyperspace_Font; curveSegments: 6;"></a-mixin>
