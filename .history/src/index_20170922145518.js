...
//  curveSegments: 6;
    const fontScale = 1/40;
    const textSettings = `size:${radius*fontScale}; height: ${radius*fontScale/10};`;
    const text = (text,longitude,latitude) => {
        `<a-entit>
           <a-entity text-geometry="value: ${(v*maxLongitude/360)+longitudeUnit}; ${textSettings}" material="color: ${color}; opacity: ${opacity*2}; transparent: true;" mixin="FontGeom" spherical-coords="longitude: ${v}; latitude: 0; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}">
           </a-entity>
        </a-entity>`;
    };
    return `
        <a-entity id="${id}">
            ...
            ${_.range(0,359.999,stepLongitude).map(v=>
                `<a-entity text-geometry="value: ${(v*maxLongitude/360)+longitudeUnit}; ${textSettings}" material="color: ${color}; opacity: ${opacity*2}; transparent: true;" mixin="FontGeom" spherical-coords="longitude: ${v}; latitude: 0; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}"></a-entity>`
            ).join('')}
            ...
            ${_.without(_.range(-80,80.001,10),0).map(v=>
                `<a-entity text-geometry="value: ${v}; ${textSettings}"  material="color: ${color}; opacity: ${opacity*2}; transparent: true;" mixin="FontGeom" spherical-coords="longitude: 0; latitude: ${v}; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}"></a-entity>`    
            ).join('')}
        </a-entity>
    `;
};
