...
const coordinateGrid = ({id='grid'+(gridId++),longitudeUnit='°', longitudeClockwise=false, color='#aaa',opacity=0.1,radius=100,fontWidth=50}) => {
    ...
    const textSettings = `size:${radius/20}; height: ${radius/200}; curveSegments: 6;`;
    return `
        <a-entity id="${id}">
            ...
            ${_.range(0,359.999,stepLongitude).map(v=>
                `<a-entity text-geometry="value: ${(v*maxLongitude/360)+longitudeUnit}; ${textSettings}" mixin="FontGeom" spherical-coords="longitude: ${v}; latitude: 0; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}"></a-entity>`
            ).join('')}
            ...
            ${_.without(_.range(-80,80.001,10),0).map(v=>
                `<a-entity text-geometry="value: ${v}; ${textSettings}" mixin="FontGeom" spherical-coords="longitude: 0; latitude: ${v}; radius: ${radius}; longitudeClockwise: ${longitudeClockwise}"></a-entity>`    
            ).join('')}
        </a-entity>
    `;
};
...
const html = `
    ...
    <a-scene 
        antialias="true" 
        celestial-system="scale-time: 100000; reposition-ms: 100; 
        ...
        <a-assets>
            ...
            <a-mixin id="FontGeom" text-geometry="font: #Hyperspace_Font"></a-mixin>
