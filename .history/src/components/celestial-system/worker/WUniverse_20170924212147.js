...
export default class WUniverse {
    ...
    setEquatorialCoordinateSystem() {
        ...
        // and the position of the sun / vernal equinox (0h right ascension)
        this.setDate(VERNAL_EQUINOX_J2000);
        this.updateBodies();
        ...
        console.log('new NORTH: '+JSON.stringify(dirNorth));
        const dirSunEarth = sun.position.clone().sub(earth.position).normalize();
        const dirRightAscension6h = dirNorth.clone().cross(dirSunEarth).normalize();

        const basis = new THREE.Matrix4().makeBasis(dirRightAscension6h,dirNorth,dirSunEarth);
