...
const celestialSystem = {
    ...
    rotationEclipticToEquatorial: new Quaternion().setFromEuler(
		new THREE.Euler(Math.degToRad( - (23 + (26 / 60) + (21 / 3600)), 0, 0, 'ZYX'))),
    ...
    getObjectPositionInfo(objectName) {
        ...
        const transform = (frameId,cw,eclipticPosition) => {
            ...
            const local = eclipticPosition.clone().applyMatrix4(frame.worldMatrix);
        ...
        if (body) {
            ...
            if (objectName=='m31') {
                ...
                const frame = document.querySelector('#galactic-reference-frame');
                const inverseWM = new Matrix4().getInverse(frame.worldMatrix,true);
