...
const celestialSystem = {
    ...
    perspective(position) {
        ...
        const perspectiveDistance = this.perspectiveDistance(distance);
        const scale = distance / perspectiveDistance;
        perspectivePosition.normalize().multiplyScalar(perspectiveDistance);
        perspectivePosition.add(this.observer.positionWorld);
        return {distance,perspectivePosition,scale};
    },

    perspectiveDistance(distance) {
        const solarSystemSize = 31 * AU * KM;
        const targetSize = 100;
        return distance / solarSystemSize * targetSize;
    }
