...
const celestialSystem = {
    ...
    getObjectPositionInfo(objectName) {
        ...
        if (body) {
            ...
            if (objectEl) {
                const position = objectEl.object3D.position.clone();
                const equatorial = position.applyQuaternion(this.rotationHorizonthalToEquatorial); 
                const galactic = position.applyQuaternion(this.rotationEquatorialToGalactic);
                return {
                    azimuthal: new LLSpherical().fromVector3(position),
                    equatorial: new LLSpherical().fromVector3(equatorial),
                    galactic: new LLSpherical().fromVector3(galactic),
                };
            }            
        }
    }
