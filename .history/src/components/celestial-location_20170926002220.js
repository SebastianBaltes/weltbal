...
AFRAME.registerComponent('celestial-location', {
    ...
    tick() {
        if (this.celestialSystem) {
            ...
            const {longitude,latitude,bodyName,altitude,north} = this.celestialSystem.location;
            const body = this.celestialSystem.getBody(bodyName);
            
            const object3D = this.el.object3D;
            object3D.matrix.compose( body.position, body.quaternion, new THREE.Vector3(1,1,1) );
