...
FORK of aframe-animation-component
fixes some problems with event listening / binding
function.bind seems not to work in my environment as expected
...
AFRAME.registerComponent('animation', {
  ...
  init: function () {
    ...
    this.config = null;
    this.playAnimationBound = (...args)=>this.playAnimation(...args);
    this.pauseAnimationBound = (...args)=>this.pauseAnimation(...args);
    this.resumeAnimationBound = (...args)=>this.resumeAnimation(...args);
    this.restartAnimationBound = (...args)=>this.restartAnimation(...args);
    this.repeat = 0;
  },
  ...
  addEventListeners: function () {
    ...
    data.startEvents.map(function (eventName) {
      console.log('add event ',eventName,'on',el);
