...
AFRAME.registerComponent('celestial-location', {
    ...
    update()  {
        ...
        const standpointSpherical = LLSpherical.create(longitude, latitude, d.radius + d.altitude);
        ...
        const horizonthUpDirection = geoCenterToStandpoint.clone().normalize().multiplyScalar(-1);
        const horizonthEastDirection = northSphericalDirection.clone().cross(horizonthUpDirection).normalize().multiplyScalar(-1); 
        const horizonthNorthDirection = horizonthUpDirection.clone().cross(horizonthEastDirection).normalize().multiplyScalar(-1); 
