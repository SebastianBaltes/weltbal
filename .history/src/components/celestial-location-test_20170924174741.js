...
AFRAME.registerComponent('celestial-location-test', {
    ...
    tick() {
        if (this.celestialSystem) {
            ...
            const longitude = s => data.system=='azimuthal' ? s.longitudeCW : loc.longitude; 

            this.i=(this.i||0)+1;
            if (this.i==100) {
                console.log(data.body,_.keys(info)
                    .map(k=>k+': '+longitude(info[k]).toFixed(2)+' / '+info[k].latitude.toFixed(2)).join('\n'));
            }
