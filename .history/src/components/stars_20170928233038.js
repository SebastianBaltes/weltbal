...
import perspective from "./celestial-system/perspective";

import {AU, KM, CIRCLE, PARSEC} from './celestial-system/constants';
...
const pxRatio = 1;//(window.devicePixelRatio || 1);
...
AFRAME.registerComponent('stars', {
    ...
    init() {
        ...
        $.get(data.src).then((stars)=>{
            ...
            stars = stars.filter(star=>star[X]!=0 && star[MAG]<data.mag && new THREE.Vector3(star[X], star[Y], star[Z]).length()<99999);
    ...
    generate(stars,texture)  {
        ...
        for (let i = 0, i3 = 0; i < count; i++, i3 += 3) {
            ...
            const perspectiveDistance = perspective(distance * PARSEC);

            distanceMin = Math.min(distanceMin,perspectiveDistance);
            distanceMax = Math.max(distanceMax,perspectiveDistance);                

            starVect.normalize().multiplyScalar(perspectiveDistance*10);
            ...
            sizes[i] = data.size==-1 
                ? pxRatio * (1.5 + Math.floor(10 * (1 / mag) ** 0.2) / 10)
                : data.size;
