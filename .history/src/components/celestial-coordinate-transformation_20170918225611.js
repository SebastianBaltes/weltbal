        ...
        if (trans=='vr-horizontal') {
            this.tick = ()=>{
                if (celestialSystem.ready) {
                    const cdata = celestialSystem.data;                    
            ...
            this.tick = ()=>{
                if (celestialSystem.ready) {
                    const cdata = celestialSystem.location;                    
                    
                    const earth = celestialSystem.getBody('earth');

                    const locationQuaternion = new Quaternion().setFromEuler (
                        new THREE.Euler(
                            Math.degToRad(cdata.latitude),
                            Math.degToRad(cdata.longitude) - earth.getCurrentRotation() - Math.degToRad(103.5),
