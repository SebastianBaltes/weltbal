	...
	*/
	...
	applyTransformationTo(object3D, withScale = true) {
		...
		if (this.name=='moon') {
			...
			console.log(this.logCounter,this.name,object3D.position.clone(),object3D.getWorldPosition(),object3D.quaternion.clone(),object3D.getWorldQuaternion());			
		}
		if (withScale) {
			const s = this.universe.data.scale * this.universe.data.scaleBodies;
	...
	applyLocationOnBodyTransformation(object3D, longitute, latitude, altitude, north) {
		...
		const scaleBodies = this.universe.data.scale * this.universe.data.scaleBodies;
		const radius = (this.radius * KM + altitude) * scaleBodies;
