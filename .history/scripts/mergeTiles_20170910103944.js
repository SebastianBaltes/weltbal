...
const depth = 3;
const outputSizes = [2048,1024,512];
const format = "jpg";
...
async function generateCompositeImages(inDir, outDir) {
    ...
    const exampleImage = await Jimp.read(inDir + tilePath(depth, 0));
...
async function slurpAllHipsFolders() {
    ...
    for (let dir of hibsFolders) {
        ...
        if (!(/jpeg/.test(properties['format']))) {
            continue;
        }

        console.log('has jpeg and properties, new name: '+name);
