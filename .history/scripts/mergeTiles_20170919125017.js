...
async function generateCompositeImages(inDir, outDir) {
    ...
    let imageFile = inDir + tilePath(depth, 0);
    const possibleFiles = ['jpg','png'].map(f=>imageFile+'.'+format);
    imageFile = possibleFiles.find(f=>fs.existsSync(f));
    if (!imageFile) {
        console.log('found none: ',possibleFiles);
        return;
    }
    ...
    for (let i=0;i<12;i++) {
        ...
        for (let tile of list) {
            const tileImage = await Jimp.read(inDir + tilePath(tile.d, tile.i));
